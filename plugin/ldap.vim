...
class ActiveDirectoryLDAP(object):
    ...
    def Search(self, username, base=DEFAULT_BASE):
        resID = self.__ld.search(base, ldap.SCOPE_SUBTREE, "mailNickname=%s*" % username)
        retVal = None
        rt,rd=self.__ld.result(resID, 0)
        if rd != []:
            retVal = '"%s" <%s>' % (rd[0][1]['name'][0], rd[0][1]['mail'][0])
        else: retVal = username + 'NF'
        #end if
